...
const NAV = [
  {
    group: "Overview",
    items: [{ label: "Dashboard", href: "/principal", icon: LayoutDashboard }],
  },
  {
    group: "Evaluations",
    items: [
      { label: "Pending Reviews",   href: "/principal/reviews",   icon: ClipboardCheck },
      { label: "Completed Reviews", href: "/principal/completed", icon: CheckCircle2   },
    ],
  },
  {
    group: "Management",
    items: [
      { label: "Manage Users", href: "/principal/users", icon: Users },
      { label: "Category Assignment", href: "/principal/faculty-category-assignment", icon: CheckSquare },
    ],
  },
  {
    group: "Account",
    items: [
      { label: "My Profile", href: "/principal/profile", icon: User },
    ],
  },
];
  ...
  return (
    <div className="flex flex-col h-full select-none bg-white border-r border-slate-200">
      ...
      <div className="flex items-center gap-3 px-6 py-6 border-b border-slate-100">
        ...
        <div>
          <p className="text-[14px] font-bold text-slate-800 tracking-tight">Principal Portal</p>
      ...
      <div className="mx-4 mt-5 mb-2 px-4 py-3.5 rounded-xl bg-slate-50 border border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-bold text-slate-800 truncate">{name || "Principal"}</p>
            <p className="text-[11px] text-slate-500 truncate">School Administrator</p>
          </div>
        </div>
        {school && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-200/60">
            ...
            <p className="text-[11px] text-slate-500 font-medium truncate">{school}</p>
          </div>
        )}
      </div>
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block fixed top-0 left-0 w-[260px] h-screen z-30">
        <NavContent pathname={pathname} name={n} school={s} onLogout={() => setShowModal(true)} />
      </aside>
      ...
      <div className="lg:hidden">
        <div className="fixed top-0 inset-x-0 z-40 h-16 bg-white border-b border-slate-200 flex items-center justify-between px-5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[15px] font-bold text-slate-800">Principal Portal</span>
          </div>
        ...
        <aside className={`fixed top-0 left-0 z-50 h-full w-[260px] bg-white transition-transform duration-300 ease-in-out ${open ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
          <button onClick={() => setOpen(false)} className="absolute top-5 right-4 text-slate-400 hover:text-slate-800 bg-slate-50 p-1.5 rounded-lg"><X size={16} /></button>
          <NavContent pathname={pathname} name={n} school={s} onNav={() => setOpen(false)} onLogout={() => { setOpen(false); setShowModal(true); }} />
        </aside>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
            <div className="p-6">
              <div className="w-12 h-12 rounded-full bg-[#e31e24]/10 flex items-center justify-center mb-4"><LogOut size={20} className="text-[#e31e24]" /></div>
              <h3 className="text-[18px] font-bold text-slate-800 mb-1.5">Sign Out</h3>
              <p className="text-[14px] text-slate-500 mb-6">Are you sure you want to sign out? You will need to log back in to access the portal.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-[14px] font-semibold text-slate-600 hover:bg-slate-50 transition">Cancel</button>
                <button onClick={logout} className="flex-1 py-2.5 rounded-xl bg-[#e31e24] text-white text-[14px] font-semibold hover:bg-[#c9181f] transition shadow-sm shadow-[#e31e24]/20">Sign Out</button>
              </div>
            </div>
          </div>
        </div>
